...
namespace Nekoyume.Model.Stat
{
    ...
    public class Stats2 : IStats, ICloneable
    {
        ...
        {
            Hp.SetValue(statsArray.Sum(stats => stats.HP));
            Atk.SetValue(statsArray.Sum(stats => stats.ATK));
            Def.SetValue(statsArray.Sum(stats => stats.DEF));
            Cri.SetValue(statsArray.Sum(stats => stats.CRI));
            Hit.SetValue(statsArray.Sum(stats => stats.HIT));
            Spd.SetValue(statsArray.Sum(stats => stats.SPD));
            Drv.SetValue(statsArray.Sum(stats => stats.DRR));
            Drr.SetValue(statsArray.Sum(stats => stats.DRV));
            Cdmg.SetValue(statsArray.Sum(stats => stats.CDMG));
        }
            ...
            foreach (var statModifier in statModifiers)
            {
                switch (statModifier.StatType)
                {
                    case StatType.HP:
                        ...
                        break;
                    case StatType.ATK:
                        Atk.AddValue(statModifier.GetModifiedPart2(baseStatsArray.Sum(stats => stats.atk.Value)));
                        break;
                    case StatType.DEF:
