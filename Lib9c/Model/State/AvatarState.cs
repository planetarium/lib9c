...
namespace Nekoyume.Model.State
{
    ...
    public class AvatarState : State, ICloneable
    {
            ...
            : base(serialized)
        {
            ...
            stageMap = new CollectionMap(serialized[stageMapKey]);
            serialized.TryGetValue((Text)monsterMapKey, out var value2);
            monsterMap = value2 is null ? new CollectionMap() : new CollectionMap(value2);
            itemMap = new CollectionMap(serialized[itemMapKey]);
            eventMap = new CollectionMap(serialized[eventMapKey]);
        ...
        public AvatarState(List serialized) : base(serialized[0])
        {
            ...
            stageMap = new CollectionMap(serialized[12]);
            monsterMap = new CollectionMap(serialized[13]);
            itemMap = new CollectionMap(serialized[14]);
            eventMap = new CollectionMap(serialized[15]);
