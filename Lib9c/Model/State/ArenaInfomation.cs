...
{
    public class ArenaInfomation : IState
    {
        ...
        public ArenaInfomation(AvatarState avatarState, CharacterSheet characterSheet, bool active)
        {
        ...
        public ArenaInfomation(AvatarState avatarState, CharacterSheet characterSheet, CostumeStatSheet costumeStatSheet, bool active)
            : this(avatarState, characterSheet, active)
        {
            CombatPoint = CPHelper.GetCPV2(avatarState, characterSheet, costumeStatSheet);
        }

        public ArenaInfomation(Dictionary serialized)
        {
        ...
        public ArenaInfomation(ArenaInfomation prevInfomation)
        {
            AvatarAddress = prevInfomation.AvatarAddress;
            AgentAddress = prevInfomation.AgentAddress;
            ArmorId = prevInfomation.ArmorId;
            Level = prevInfomation.Level;
            AvatarName = prevInfomation.AvatarName;
            CombatPoint = prevInfomation.CombatPoint;
            Score = 1000;
            DailyChallengeCount = GameConfig.ArenaChallengeCountMax;
            Active = false;
            ArenaRecord = new Record();
        ...
        public ArenaInfomation Clone() => new ArenaInfomation(this)
        {
            Score = Score,
        ...
        [Obsolete("Use Update()")]
        public int UpdateV3(AvatarState avatarState, ArenaInfomation enemyInfomation, BattleLog.Result result)
        {
            switch (result)
            {
            ...
            var score = ArenaScoreHelper.GetScoreV1(Score, enemyInfomation.Score, result);
        ...
        [Obsolete("Use Update()")]
        public int UpdateV4(ArenaInfomation enemyInfomation, BattleLog.Result result)
        {
            DailyChallengeCount--;
            switch (result)
            {
            ...
            var earnedScore = ArenaScoreHelper.GetScoreV2(Score, enemyInfomation.Score, result);
            Score = Math.Max(1000, Score + earnedScore);
            return earnedScore;
        }

        [Obsolete("Use Update()")]
        public int UpdateV5(ArenaInfomation enemyInfomation, BattleLog.Result result)
        {
            DailyChallengeCount--;
            switch (result)
            {
            ...
            var score =
                ArenaScoreHelper.GetScoreV3(Score, enemyInfomation.Score, result);
            Score = Math.Max(1000, Score + score.challengerScore);
            enemyInfomation.Score = Math.Max(1000, enemyInfomation.Score + score.defenderScore);
        ...
        public int Update(
            ArenaInfomation enemyInfomation,
            ...
            Func<int, int, BattleLog.Result, (int challengerScore, int defenderScore)> scoreGetter)
        {
            ...
            var (challengerScore, defenderScore) = scoreGetter(Score, enemyInfomation.Score, result);
            Score = Math.Max(1000, Score + challengerScore);
            enemyInfomation.Score = Math.Max(1000, enemyInfomation.Score + defenderScore);
