...
{
    [ActionType("transfer_to_garages")]
    public class TransferToGarages : GameAction, ITransferToGarages, IAction
    {
        public IOrderedEnumerable<(Address balanceAddr, FungibleAssetValue value)>?
            FungibleAssetValues { get; private set; }

        /// <summary>
        /// The sender's inventory address.
        ...
        /// </summary>
        public Address? SenderInventoryAddr { get; private set; }
            ...
            {
                {
                    ...
                    new List(
                        ...
                        SenderInventoryAddr is null
                            ? Null.Value
                            : SenderInventoryAddr.Serialize(),
            ...
            string addressesHex)
        {
            ...
            if (!SenderInventoryAddr.HasValue)
            {
                throw new InvalidActionFieldException(
                    $"[{addressesHex}] {nameof(SenderInventoryAddr)} is required when " +
                    $"{nameof(FungibleIdAndCounts)} is set.");
            }

            if (!Addresses.CheckInventoryAddrIsContainedInAgent(signer,
                    SenderInventoryAddr.Value))
            {
                throw new InvalidActionFieldException(
                    innerException: new InvalidAddressException(
                        $"[{addressesHex}] {signer} doesn't have permission for {SenderInventoryAddr}."));
            }
        ...
        private IAccountStateDelta TransferFungibleItems(
            ...
            IAccountStateDelta states)
        {
            if (SenderInventoryAddr is null ||
                FungibleIdAndCounts is null)
            {
                return states;
            }

            var inventory = states.GetInventory(SenderInventoryAddr.Value);
            ...
            foreach (var (fungibleId, count, garageAddr, garageState) in fungibleItemTuples)
            {
                if (!inventory.TryGetTradableFungibleItems(
                        ...
                        out var outItems))
                {
                    throw new ItemNotFoundException(SenderInventoryAddr.Value, fungibleId);
                }
            ...
            return states.SetState(SenderInventoryAddr.Value, inventory.Serialize());
        }
    }
}
