...
#nullable enable
    ...
    public class TransferToGaragesTest
    {
            ...
            (HashDigest<SHA256> fungibleId, int count)[] fungibleIdAndCounts)
        {
            ...
            foreach (var action in actions)
            {
                ...
                Assert.Equal(action.SenderInventoryAddr, des.SenderInventoryAddr);
                ...
                var actionInter = (ITransferToGarages)action;
                var desInter = (ITransferToGarages)des;
                Assert.True(
                    actionInter.FungibleAssetValues?.SequenceEqual(desInter.FungibleAssetValues!) ??
                    desInter.FungibleAssetValues is null);
                Assert.Equal(actionInter.SenderInventoryAddr, desInter.SenderInventoryAddr);
                Assert.True(
        ...
        public void Execute_Success()
        {
            ...
            if (action.SenderInventoryAddr is null ||
                action.FungibleIdAndCounts is null)
            {
                return;
            }

            var inventoryState = nextStates.GetState(action.SenderInventoryAddr.Value)!;
        ...
        private static (TransferToGarages action, IAccountStateDelta nextStates) Execute(
            Address signer,
            long blockIndex,
        ...
        {
            var action = new TransferToGarages(
                fungibleAssetValues,
                inventoryAddr,
                fungibleIdAndCounts);
            return (
            ...
            GetSuccessfulPreviousStatesWithPlainValue()
        {
            ...
            var fungibleAssetValues = GetFungibleAssetValues(AgentAddr, _avatarAddress);
