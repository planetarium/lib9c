...
namespace Lib9c.Tools.SubCommand
{
    public class Store
    {
        ...
        public void Migration(
            [Option('s', Description = "Path to migration target root path.")]
            ...
            )
        {
            if (!skipCopy)
            {
                ...
                DirectoryCopy(sourceRootPath, distRootPath, true);
            ...
            var sourceStore = new OldRocksDBStore(sourceRootPath);
            var distStore = new RocksDBStore(distRootPath);

            var totalLength = sourceStore.CountBlocks();
            if (!skipBlock)
            {
                Console.WriteLine("Start migrate block.");
                foreach (var item in
                    sourceStore.IterateBlockHashes().Select((value, i) => new {i, value}))
                {
                    Console.WriteLine($"block progress: {item.i}/{totalLength}");
                    distStore.PutBlock(sourceStore.GetBlock<NCAction>(item.value));
                }

                Console.WriteLine("Finish migrate block.");                
            }

            totalLength = sourceStore.CountTransactions();
            if (!skipTransaction)
            {
                Console.WriteLine("Start migrate transaction.");
                foreach (var item in
                    sourceStore.IterateTransactionIds()
                        .Select((value, i) => new {i, value}))
                {
                    Console.WriteLine($"tx progress: {item.i}/{totalLength}");
                    distStore.PutTransaction(sourceStore.GetTransaction<NCAction>(item.value));
                }
